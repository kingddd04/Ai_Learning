...
for epoch in range(epochs):
    ...
    for index in range(0, n_samples, batch_size):
        ...
        d4 = (A4 - Yb) 
        dW4 = A3.T @ d4
        db4 = d4.sum(axis=0, keepdims=True)

    
        d3 =  (W4 @ d4) * relu_deriv(Z3)
        dW3 = d3 @ A3.T
        db3 = d3.sum(axis=0 , keepdims=True)

        d2 = (W3 @ d3)* relu_deriv(Z2.T)
        dW2 = A3 @ d2.T
        db2 = d2.sum(axis=0 , keepdims=True)

        d1 = (W2 @ d2) * relu_deriv(Z1.T)
        print(d1.shape , Xb.shape)
        dW1 = d1 @ Xb
        db1 = d1.sum(axis=0, keepdims=True)

        print("-----------", A2.shape, "------------", dW2.shape)
...
"""Traceback (most recent call last):
  errore[...] line 112, in <module>    
    W2 -= (lr * dW2).T
ValueError: operands could not be broadcast together with shapes (6,8) (5,1) (6,8)


per il layer 2 ce un problema che si nota solo quando si applica grad desc
il dw dei pesi del layer 2 ha una shape sbagliata rispetto a w2 nonostante abbia applicato 
le formule nelle slides alla perfezione per il layer

il layer due correttamente produce un vettore di output di shape (1, 8) come del resto ma il dW2 risulta produrre una shape di (1, 5) inadattabile
